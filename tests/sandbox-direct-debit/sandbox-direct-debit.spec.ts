...
const pathname = '/documentation/introduction/sandbox-guideline/direct-debit/'
const baseURL = url + pathname

test.describe('Sandbox direct debit page', () => {
  ...
  test('developers sandbox direct debit page has title', async ({ page }) => {
    await page.goto(baseURL)
    await expect(page).toHaveTitle(/Krungthai Developers/)
  })

  test('take baseline screenshot of developers sandbox direct debit page', async ({ page }) => {
    const screenshotPath = path.join(process.cwd(), '_screenshot_', 'sandbox-direct-debit-baseline.png')
  ...
  test('Screenshots of developers sandbox direct debit page', async ({ page }) => {
    await page.goto(baseURL, { waitUntil: 'load', timeout: timeout })

    // Additional wait for any dynamic content
    await page.waitForTimeout(waitTimeout)

    const screenshotPath = path.join(process.cwd(), 'screenshot', 'sandbox-direct-debit-full.png')
