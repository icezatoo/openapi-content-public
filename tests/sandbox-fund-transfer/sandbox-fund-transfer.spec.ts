...
const pathname = '/documentation/introduction/sandbox-guideline/fund-transfer/'
const baseURL = url + pathname

test.describe('Sandbox fund transfer page', () => {
  ...
  test('developers sandbox fund transfer page has title', async ({ page }) => {
    await page.goto(baseURL)
    await expect(page).toHaveTitle(/Krungthai Developers/)
  })

  test('take baseline screenshot of developers sandbox fund transfer page', async ({ page }) => {
    const screenshotPath = path.join(process.cwd(), '_screenshot_', 'sandbox-fund-transfer-baseline.png')
  ...
  test('Screenshots of developers sandbox fund transfer page', async ({ page }) => {
    await page.goto(baseURL, { waitUntil: 'load', timeout: timeout })

    // Additional wait for any dynamic content
    await page.waitForTimeout(waitTimeout)

    const screenshotPath = path.join(process.cwd(), 'screenshot', 'sandbox-fund-transfer-full.png')
